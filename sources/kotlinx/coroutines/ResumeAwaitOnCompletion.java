...
final class ResumeAwaitOnCompletion<T> extends JobNode {
    ...
    public void invoke(Throwable cause) {
        ...
        if (DebugKt.getASSERTIONS_ENABLED() && ((state instanceof Incomplete) ^ 1) == 0) {
            ...
            this.continuation.resumeWith(Result.m61constructorimpl(ResultKt.createFailure(((CompletedExceptionally) state).cause)));
        } else {
            Result.Companion companion2 = Result.Companion;
            this.continuation.resumeWith(Result.m61constructorimpl(JobSupportKt.unboxState(state)));
        }
    }
}
