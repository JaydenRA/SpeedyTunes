...
public final class SelectBuilderImpl<R> extends LockFreeLinkedListHead implements SelectBuilder<R>, SelectInstance<R>, Continuation<R>, CoroutineStackFrame {
        ...
        if (!DebugKt.getASSERTIONS_ENABLED() || isSelected() != 0) {
            while (true) {
                ...
                if (result$iv == SelectKt.UNDECIDED) {
                    ...
                    if (Result.m67isFailureimpl(result)) {
                        Continuation<R> continuation = this.uCont;
                        Throwable exception$iv = Result.m64exceptionOrNullimpl(result);
                        ...
                        continuation.resumeWith(Result.m61constructorimpl(ResultKt.createFailure(th)));
                        return;
                    }
        ...
        if (!DebugKt.getASSERTIONS_ENABLED() || isSelected() != 0) {
            while (true) {
                ...
                if (result$iv == SelectKt.UNDECIDED) {
                    ...
                    Result.Companion companion = Result.Companion;
                    intercepted.resumeWith(Result.m61constructorimpl(ResultKt.createFailure(exception)));
                    return;
                }
            }
        } else {
    ...
    public final void handleBuilderException(Throwable e) {
        if (trySelect()) {
            Result.Companion companion = Result.Companion;
            resumeWith(Result.m61constructorimpl(ResultKt.createFailure(e)));
