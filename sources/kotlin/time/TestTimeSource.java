...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1508plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1420toLongimpl(duration, getUnit());
        if (longDelta == Long.MIN_VALUE || longDelta == Long.MAX_VALUE) {
            double newReading2 = ((double) this.reading) + Duration.m1417toDoubleimpl(duration, getUnit());
    ...
    private final void m1507overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + "ns is advanced by " + Duration.m1423toStringimpl(duration) + '.');
    }
}
